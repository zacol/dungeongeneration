...
EasyStar.js = function() {
  ...
  this.findPath = function(startX, startY, endX, endY, callback) {
    ...
    instance.openList = new EasyStar.PriorityQueue(
      'bestGuessDistance',
      EasyStar.PriorityQueue.MIN_HEAP,
    );
  ...
  var checkAdjacentNode = function(instance, searchNode, x, y, cost) {
    ...
    if (
      instance.endX === adjacentCoordinateX &&
      instance.endY === adjacentCoordinateY
    ) {
    ...
    if (
      pointsToAvoid[adjacentCoordinateX + '_' + adjacentCoordinateY] ===
      undefined
    ) {
      for (var i = 0; i < acceptableTiles.length; i++) {
        if (
          collisionGrid[adjacentCoordinateY][adjacentCoordinateX] ===
          acceptableTiles[i]
        ) {

...
export class MapFactory {
  ...
  generateRooms() {
    ...
    this.generatePath(
      'path',
      this.map.roomsToExit,
      this.map.settings.maxMainRooms,
    );

    this.generatePath(
      'random',
      this.map.mediumRooms,
      this.map.settings.maxMediumRooms,
    );

    this.generatePath(
      'randomMedium',
      this.map.hardRooms,
      this.map.settings.maxHardRooms,
    );
  }
   ...
   * @private
   */
  generatePath(type, roomList, maxRooms) {
    ...
    while (roomList.length < maxRooms) {
      ...
      while (true) {
        ...
        if (directions.length === 0) {
          ...
          switch (type) {
            ...
            case 'path':
              ...
              const mediumRoom = this.map.roomsToExit.splice(
                this.map.roomsToExit.length - 1,
                1,
              )[0];

              // The current room failed to expand in any direction, this is why we promote it to a medium room. This means the room isn't part of the main route and is 'optional'.
        ...
        const side = Utils.randomNumber(0, directions.length - 1);
        const direction = directions[side];

        const maxDirectionTries = 5;
        let directionTries = 0;

        let newRoom = this.generateRoomNextTo(prevRoom, direction);
   ...
   */
  generateRoomNextTo(prevRoom, direction) {
    let heigth, width, xPos, yPos;
   ...
   * @return {Room} A new Room object.
   */
  newRoom(width, height, xPos, yPos) {
    const w =
    ...
    const x =
      typeof xPos === 'undefined'
        ? Utils.randomNumber(1, this.game.map.settings.tilesX - w - 1)
        : xPos;

    const y =
      typeof yPos === 'undefined'
        ? Utils.randomNumber(1, this.game.map.settings.tilesY - h - 1)
        : yPos;
  ...
  placeRoom(room) {
    ...
    for (let x = room.x1; x < room.x2; x++) {
      let layoutXPos = room.x2 - x - 1;

      for (let y = room.y1; y < room.y2; y++) {
        const layoutYPos = room.y2 - y - 1;
   ...
   */
  generateCorridor(firstRoom, secondRoom) {
    const firstExit = {
    ...
    if (secondExit.x !== firstExit.x) {
      ...
      if (secondExit.x - firstExit.x > 0) {
        // Corridor going left.
        for (let i = secondExit.x; i >= firstExit.x; i--) {
          this.generateHorizontalCorridor(i, secondExit.y);
        }
      } else {
        // Corridor going right.
        for (let b = secondExit.x; b <= firstExit.x; b++) {
    ...
    if (secondExit.y !== firstExit.y) {
      ...
      if (secondExit.y - firstExit.y > 0) {
        ...
        for (let n = secondExit.y; n >= firstExit.y; n--) {
          this.generateVerticalCorridor(firstExit.x, n);
        }
      } else {
        // If the corridor is going down.
        for (let h = secondExit.y; h <= firstExit.y; h++) {
   ...
   */
  generateHorizontalCorridor(x, y) {
    const map = this.game.map;

    const currentTile = map.tiles[x][y];

    const aboveTile = map.tiles[x][y + 1];
    const belowTile = map.tiles[x][y - 1];

    // If the current tile type is a wall, and the tiles above and below here are also walls this may be a possible door location.
    if (
      currentTile.type === 1 &&
      aboveTile.type === 1 &&
      belowTile.type === 1
   ...
   * @param {Number} y - The vertical position of the tile that has to become a corridor.
   */
  generateVerticalCorridor(x, y) {
    const map = this.game.map;

    const currentTile = map.tiles[x][y];

    const rightTile = map.tiles[x + 1][y];
    const leftTile = map.tiles[x - 1][y];

    // If the current tile type is a wall, and the tiles left and right here are also walls this may be a possible door location.
    ...
    // Because we don't know which direction the vertical corridor is being placed, we check upperleft, upperright, bottomleft and bottomright.
